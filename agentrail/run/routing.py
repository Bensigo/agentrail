...
next-tier step used by the escalation loop.
...
2. **Tier step** (``next_tier``): the pure cheap→strong step used by the live
   escalation loop in ``agentrail/heartbeat/runtime.py``. The loop calls
   ``budget_leash.check``, ``next_tier``, and ``compaction.build`` directly.
"""
