...
class PromotionPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        unknown = [key for key in self.floors if key[0] not in SCORECARDS or not key[1]]
        if unknown:
            raise ValueError("promotion floors must name a known scorecard and non-empty segment")
        if self.required_arms != ARMS:
...
def evaluate_promotion(
    ...
    *,
    ...
    A missing/unknown case split, missing cell, unscored-only cell, or too-small
    denominator is a ``hold``.  A measured violation of a declared safety or
    quality floor is a ``reject``.  Only complete held-out evidence for every
    required arm/floor pair can promote.
    """
    ...
    for arm in policy.required_arms:
        for (scorecard, segment), floor in policy.floors.items():
            ...
            if scored < floor.min_scored:
                holds.append(f"{label} has {scored}/{floor.min_scored} independently scored held-out observations")
                continue
            if rates["false_green_rate"] > floor.max_false_green_rate:
                rejects.append(f"{label} false-green rate {rates['false_green_rate']:.3f} exceeds {floor.max_false_green_rate:.3f}")
            if rates["false_block_rate"] > floor.max_false_block_rate:
                rejects.append(f"{label} false-block rate {rates['false_block_rate']:.3f} exceeds {floor.max_false_block_rate:.3f}")
            if rates["correct_rate"] < floor.min_correct_rate:
                rejects.append(f"{label} correct rate {rates['correct_rate']:.3f} is below {floor.min_correct_rate:.3f}")
