...
1. Run a criterion-specific UI flow against a safe exact-head environment.
   The local API worker has completed an artifact-backed loopback proof; repeat
   it on a deployed PR environment, then run bounded data verification with
   configured artifact storage. Add a job trigger/output executor only when a
   safe bounded job surface exists, otherwise retain `not_testable` rather than
   forcing those modalities through screenshots.
