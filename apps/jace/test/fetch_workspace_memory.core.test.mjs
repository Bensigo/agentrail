...
test("buildMemoryUrl targets the workspace-memory route with NO query params", () => {
  const url = buildMemoryUrl("https://c.example.com");
  assert.equal(url, `https://c.example.com${MEMORY_PATH}`);
  // The workspace is derived from the token server-side — no query string ever.
...
test("fetchWorkspaceMemory returns projected items + count on 200 (ok path)", async () => {
  ...
  // Exactly one attempt, with the bearer + accept headers, to the no-query URL.
