...
- [x] **Step 1:** Env + docs: add `PREVIEW_BOOTS_ENABLED`, `PREVIEW_BOOTS_WORKSPACES`, `PREVIEW_BOOT_TTL_SECONDS`, `PREVIEW_WORKER_ENABLED`, `PREVIEW_ADVERTISE_HOST`, `JACE_CONSOLE_TOKEN` (worker side) to the relevant `.env.example`/`deploy/.env.production.example` with one-line comments; a short `deploy/` note: the preview worker is a new out-of-process service; the browser sidecars + worker share the Railway private network; the child binds `0.0.0.0`, advertised via `*.railway.internal`, never public. Verified in `.env.example`, `deploy/.env.production.example`, and `deploy/preview-worker/README.md`.
- [x] **Step 2:** Full suites: `DATABASE_URL=... pnpm --filter @agentrail/db-postgres test`; console suite; `pytest agentrail/sandbox/tests/test_preview_recipe.py agentrail/sandbox/tests/test_preview_boot.py agentrail/runner/tests/test_preview_worker.py`. All green. GitHub CI run `30801089086` passed Node (including live Postgres migration/database tests), Python, and shell; the local console suite also passed 283 files / 5,559 tests.
...
**Current deployment gate (2026-08-03):** The Railway `agentrail/production` environment is still running `main` and has only the existing console, fleet, Jace, Postgres, and ClickHouse services. It has no dedicated preview-worker service, preview-boot flags/enrollment, or S3-compatible artifact-storage configuration. The live smoke and the dependent reviewer-job smoke therefore remain intentionally unchecked; creating or duplicating a Railway environment must happen as an explicit deployment operation before this PR can merge.
