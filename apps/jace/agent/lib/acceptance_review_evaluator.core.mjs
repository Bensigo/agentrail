...
export function createAcceptanceReviewEvaluator({ fetchEvidence, generate }) {
  ...
  return async (item) => {
    ...
    const runtimeEvidence = Array.isArray(item.runtimeEvidence) ? item.runtimeEvidence : [];
    const output = await generate({
      ...
      instruction: `Return only criterion results and blocking findings requiring code changes. Never emit style advice. Cite only provided exact-head diff lines. The following runtime metadata was resolved by the Console from persisted exact-head criterion executions and artifacts: ${JSON.stringify(runtimeEvidence)}. A user-visible criterion may be proven only when its matching resolved runtime entry has executionStatus \"proven\" and at least one artifact. Copy runtime evidence only from this metadata; never invent it. Without matching safe proof, return not_proven.`,
    });
