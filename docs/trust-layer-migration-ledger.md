...
| Criterion execution queue, guarded result seam, and opt-in Eve worker | `ee6f36d7` through `ec9bfc08`, `fbef412f`, `0a615e7f`, current safe-preview queue slice | Focused runner admission/completion, artifact, plan, prompt, worker-core, worker-runtime, console-client, migration-schema, DB proof-modality tests; DB query typecheck. The worker claims only plan-bound exact-head UI/API jobs whose matching preview is `ready` with a URL. A planned UI criterion now persists a bounded, immutable `uiSteps` list (`open` relative path, `click`, `fill`, bounded key press, `expect_text`, and decisive `screenshot` only); Console admission rejects absent, unknown, external, oversized, extra-payload, or over-budget steps. The claim carries those exact actions to the worker. Historical UI plans without them terminate `not_testable` before an executor is invoked. A `pending`, `claimed`, or `booting` preview stays queued rather than becoming a fabricated `not_testable`; superseded, missing, mismatched, failed, or torn-down previews become explicitly `not_testable`, and a lost post-claim projection is terminalized. A no-id claim re-runs the same terminal-preview reconciliation, closing the narrow pre-claim race without treating a transient boot as terminal. UI claims now bypass root Jace and QA: a dedicated Streamable-HTTP MCP client lists the required agent-browser tools, makes only the persisted open/click/fill/press/wait/get-URL/screenshot calls in an isolated session, rejects missing sidecar/tool contracts, checks the actual reported URL remains on the exact preview origin before capture, and uploads only returned PNG/JPEG bytes through the plan-bound artifact route. A `proven` result must use artifacts from that same plan with the planned proof type: PNG/JPEG for UI and JSON for API. Until dedicated safe executors and artifact types exist, the route and DB query both refuse a planned `job` or `data` modality, and the proof seam refuses to reinterpret either as UI. Such criteria must be recorded as `not_testable` with a reason. Production Compose now contains private agent-browser sidecar and internal Jace URL wiring, while worker activation remains an explicit Console-token/artifact-store opt-in. No service has been deployed and no safe-preview UI flow has run live. API execution still uses the pre-existing constrained Eve/QA path. |
...
The next runtime-proof slice must run this direct executor against a planned
safe UI flow and bind its observed result to these artifacts. Delivery is currently queue plus
