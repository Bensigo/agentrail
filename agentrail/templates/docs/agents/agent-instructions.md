...
Read `.agentrail/context.md` before making non-trivial changes. Then read `.agentrail/taste.md` when present, especially for product, UI, copy, interaction, or visual evidence decisions. If domain docs exist, prefer them over assumptions.
...
AgentRail also installs curated first-party skills for common implementation contexts, such as frontend web, Tauri desktop, backend APIs, devops/deploy work, and current-docs verification. These are reviewed local files under `.agentrail/skills/`, not arbitrary third-party hot installs.
...
2. Read `.agentrail/taste.md` when present and relevant to product quality, UI, copy, interaction, or visual evidence.
