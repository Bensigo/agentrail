...
**ARC B REVIEW FIX WAVE (per-job session restructure) — amends this task's shipped behavior.** Claim binding an eve session unconditionally meant the worker had to open a real session (a real, if minimal, model turn) BEFORE every claim attempt, including every idle poll that found nothing eligible — 2,880 turns/day at rest, reviewed Important-severity once the worker's actual session-minting mechanism (`apps/jace/agent/lib/review_job_worker.mjs`) was concrete. Binding moved out of claim and into its own route:
- **claim** (`.../claim/route.ts`): body is now `{workerId}` ONLY — no `eveSessionId`, no `bindReviewJobSession` call, no release-on-bind-failure branch. Claimed → 200 `{job: {...}}` unchanged; no eligible job → 204 unchanged.
- **NEW `.../bind/route.ts`:** `POST {jobId, eveSessionId}` (both required, 400 otherwise), same auth. Precondition check via the new `getReviewJobState(jobId)` query (`packages/db-postgres` — purely additive, does not modify `bindReviewJobSession`/`releaseReviewJob`'s own contracts): `state !== "running"` (covers both "no such job" and "exists but some other state," mirroring `complete`'s own "unknown OR not-running" precedent) → 409, `bindReviewJobSession` never called. Otherwise calls `bindReviewJobSession({jobId, eveSessionId})`; on throw → `releaseReviewJob({jobId})` (best-effort, logs both failures if the release ALSO fails) → 503 — the exact compensating pattern the original claim route used, relocated verbatim. Success → 200 `{ok:true}`.
- The Jace worker now calls `.../bind` itself, once a session is opened for an actual claimed job, before sending the real review turn — see Task 5/6's own amendments below for the loop-order change this requires.
...
**Interfaces (produces) — AMENDED by the Arc B review fix wave (per-job session restructure); the block below is the CURRENT contract, not the originally-shipped one:**
```js
export function createReviewJobWorker({
  claim,      // async () => job | null — NO arguments; no eveSessionId to carry at claim time anymore
  bind,       // async ({jobId, eveSessionId}) => void — NEW seam, replaces the eveSessionId that used to ride inside claim
...
```
Loop contract (each a test with fakes; ORDER per the fix wave — claim is now first and cheap, a session only ever opens for an actual claimed job): `start()` twice → second is a no-op (double-start guard); `tick()` with no job (`claim()` resolves null) → "idle", **NO session opened, NO bind, NO complete — an idle poll now mints zero model turns** (this restores the original brief prose literally: the FIRST shipped version of this module opened a session unconditionally before claiming, which the fix wave replaced); happy path → `claim()` (no args) → `openSession()` → `bind({jobId, eveSessionId: session.id})` → `send(promptFor(job), schema)` → `complete(outcome:"posted", ...)` → `close()` → "done"; `bind()` throwing → closes the session, does **NOT** call `complete` (the job's server-side state is no longer this worker's to resolve — a 503 already released it, a 409 means it's no longer running), logged, → "failed", loop alive; `openSession()` throwing AFTER a successful claim → **IS** reported via `complete(outcome:"failed")` (the job is still genuinely running under this claim, unlike a bind failure) → "failed"; send rejects or exceeds `jobTimeoutMs` → complete `outcome:"failed"` with the error string, session closed, → "failed", **and the loop keeps running** (next `tick` still fires); claim throws → logged, "idle", loop alive, never reaches `openSession`/`bind`/`complete`; complete throws after a posted result → logged, loop alive (console's stale-requeue is the safety net); `stop()` halts the interval. NOTHING may escape: every `tick` path resolves, never rejects (assert via a rejection-tripwire in tests).
...
**ARC B REVIEW FIX WAVE (per-job session restructure) — amends this task's shipped interfaces.** `review_job_console.mjs`'s file list gains a THIRD transport, `bindReviewJobSession({jobId, eveSessionId})`, hitting the new `POST .../review-jobs/bind` (Task 4's amendment); its `claimReviewJob` no longer accepts or sends `eveSessionId` (body is `{workerId}` only). `review_job_worker.mjs` (the assembler) gains `createBindFn` (curries `env` into `bindReviewJobSession`, mirroring `createClaimFn`/`createCompleteFn`) and wires `bind: createBindFn({env})` into the core factory call alongside the now-argument-less `claim: createClaimFn({workerId, env})`. `createOpenSessionFn`'s real-turn completeness guard is tightened to require `result.status === "completed"` (not merely `result.data !== undefined`), matching the bootstrap's own standard. The module's header comment states: (a) the bootstrap now runs once per claimed job — idle polls are model-free; (b) the mechanism is UNVERIFIED against a live eve server, naming the recommended smoke test (flag on, one enrolled test PR, watch one job end-to-end); (c) the binding-before-real-turn invariant this restructure preserves even though claim+bind are no longer one atomic call.
...
5. **Session binding (AMENDED, Arc B review fix wave):** the `bind` route binds the headless session to the job's workspace before the review turn; idle polls open no session and mint no model turn. Concretely: claim no longer binds — it only claims (`{workerId}`, no `eveSessionId`); the `bind` route inserts the `jace_sessions` row (workspace anchor, `channel:'review-job'`, per-job conversationKey) after checking the job is still `running` (409 otherwise); **zero diffs** in the six session-resolving tool modules; a bind failure releases the claim (503, same compensating pattern as the original design, now living in `bind`'s own route).
...
7. **Worker (order AMENDED, Arc B review fix wave):** flag-gated (`JACE_REVIEW_WORKER`), instrumentation-launched with double-start guard, never-throw tick discipline. Order is now `claim → (no job: idle, nothing further) → openSession → bind → send → complete → close` — a session opens ONLY once a job is claimed (never on an idle poll) and is bound before the real review turn, then closed after; timeout → failed + loop alive; a bind failure closes the session and fails the tick WITHOUT calling complete (the job's server-side state is no longer this worker's to resolve).
