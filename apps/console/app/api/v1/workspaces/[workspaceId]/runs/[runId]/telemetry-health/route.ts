...
export async function GET(
  ...
  try {
    const signals = await getRunTelemetryHealth(workspaceId, runId);
    return NextResponse.json({ signals });
  } catch {
    return NextResponse.json({ signals: defaultTelemetryHealthSignals() });
  }
}
