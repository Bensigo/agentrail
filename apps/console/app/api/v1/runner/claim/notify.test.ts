...
describe("notifyAccountCapacity", () => {
  it("does nothing when the workspace has no chat session bound (telegram/discord/slack)", async () => {
    mockLatestChatSession.mockResolvedValue(null);

    await notifyAccountCapacity(WS, "capacity");

    expect(mockLatestChatSession).toHaveBeenCalledWith(WS);
    expect(mockSend).not.toHaveBeenCalled();
    expect(mockSendDiscord).not.toHaveBeenCalled();
  });

  it("kind='capacity' sends the paused message into the workspace's telegram session", async () => {
    mockLatestChatSession.mockResolvedValue(TELEGRAM_SESSION as never);
  ...
  it("kind='capacity_warning' sends the warning message into the workspace's telegram session", async () => {
    mockLatestChatSession.mockResolvedValue(TELEGRAM_SESSION as never);

    await notifyAccountCapacity(WS, "capacity_warning");
  ...
  it("routes to the PLAIN discord sender — never the followup-preferring one, no interaction token exists in this async claim context", async () => {
    mockLatestChatSession.mockResolvedValue(DISCORD_SESSION as never);
  ...
  it("skips slack delivery entirely and logs loudly with the [capacity-notify] prefix — no derivable team id in this claim-route context (no live inbound payload to read one from)", async () => {
    mockLatestChatSession.mockResolvedValue(SLACK_SESSION as never);
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

    try {
      await expect(notifyAccountCapacity(WS, "capacity")).resolves.toBeUndefined();
  ...
  it("propagates a session-lookup failure — the caller (route.ts's maybeNotifyCapacity) owns the best-effort contract", async () => {
    mockLatestChatSession.mockRejectedValue(new Error("db blip"));

    await expect(notifyAccountCapacity(WS, "capacity")).rejects.toThrow("db blip");
    expect(mockSend).not.toHaveBeenCalled();
  });
  ...
  it("defensively skips and logs for a channel value it doesn't recognize (jace_sessions is filtered to telegram/discord/slack, but the return type is a plain string)", async () => {
    mockLatestChatSession.mockResolvedValue({ channel: "console", conversationKey: "c-1" } as never);
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
