...
const payload = {
  ...
  sourceReferences: [{ kind: "hosted_channel_message" }],
};

function request(body: unknown, authorized = true) {
  return new NextRequest("http://localhost/api/v1/runner/acceptance-intakes", {
    method: "POST",
    headers: {
...
beforeEach(() => {
  ...
  vi.mocked(recordAcceptanceInboundIntake).mockResolvedValue({
    ...
    message: { id: "message-1", sourceKey: "inbox-1" },
    inserted: true,
  } as never);
});
...
describe("POST /api/v1/runner/acceptance-intakes", () => {
  ...
  it("records bounded provenance only and never authorizes implementation", async () => {
  ...
  it("rejects oversized text and source-reference lists before writing", async () => {
    expect(
      (await POST(request({ ...payload, text: "x".repeat(8_001) }))).status
    ).toBe(400);
    expect(
      (await POST(
        request({ ...payload, sourceReferences: Array.from({ length: 33 }, () => ({})) })
      )).status
    ).toBe(400);
    expect(recordAcceptanceInboundIntake).not.toHaveBeenCalled();
  });
