...
// evidence; `search_events` (this file's sibling) is a secondary source —
// a change candidate can surface in an event/log line (a migration failure,
// a config-reload event) before it shows up as a discrete "change" record.
//
...
// debugger's own fetch_changes.ts injects its real driver, so the core stays
// hermetic in tests.
...
export default defineTool({
  description:
    ...
    "railway, and the always-on internal factory adapter over this app's own runs). windowStart/windowEnd " +
    "(ISO-8601, both REQUIRED — every evidence query is scoped to a window) bound the search; scope " +
    "optionally narrows within a provider (e.g. a repo or service name); limit caps how much comes back " +
