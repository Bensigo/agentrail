...
const mocks = vi.hoisted(() => {
  ...
  return {
    record: vi.fn(),
    audience: vi.fn(() => `agentrail://correction-dispatch/github-claude/ack/v1/${"a".repeat(64)}`),
    verify: vi.fn(),
    ConflictError,
  };
});
...
vi.mock("@agentrail/db-postgres", () => ({
  recordGithubClaudeAgentAcknowledgement: mocks.record,
  githubClaudeAcknowledgementAudience: mocks.audience,
  GithubClaudeAgentAcknowledgementConflictError: mocks.ConflictError,
}));

vi.mock("../../../../../../lib/github-actions-oidc", () => ({
  verifyGithubClaudeAcknowledgementOidcToken: mocks.verify,
}));
...
const claims = {
  ...
  audience: `agentrail://correction-dispatch/github-claude/ack/v1/${"a".repeat(64)}`,
  subject: "repo:Bensigo/example:ref:refs/heads/main",
  subjectSha256: "c".repeat(64),
  jtiSha256: "d".repeat(64),
  ...
  jobWorkflowRef:
    `Bensigo/agentrail/.github/workflows/github-claude-correction-ack.yml@${"f".repeat(40)}`,
...
function request(input: {
  ...
  return new Request("https://www.heyjace.com/api/v1/webhooks/github-actions/claude-ack", {
    method: "POST",
    headers,
...
beforeEach(() => {
  ...
  mocks.audience.mockReturnValue(
    `agentrail://correction-dispatch/github-claude/ack/v1/${"a".repeat(64)}`
  );
  mocks.verify.mockResolvedValue({ ok: true, claims });
  mocks.record.mockResolvedValue({ kind: "recorded", receipt: { id: "receipt" } });
});

describe("POST GitHub Claude acknowledgement", () => {
  ...
  it.each([
    ...
    ["null", { value: null }],
    ["array", { value: [] }],
    ["extra target authority", { value: { ...body, dispatchId: crypto.randomUUID() } }],
  ...
  it("requires an exact signed run binding", async () => {
  ...
  it("records only the verified projection and ephemeral session locator", async () => {
  ...
  it("returns exact replay truth without recording another provider outcome", async () => {
    mocks.record.mockResolvedValueOnce({ kind: "replayed", receipt: { id: "receipt" } });
    const response = await POST(request());
  ...
  it("maps stale/conflicting custody to 409 and storage failure to a fixed 503", async () => {
    ...
    await expect(stale.json()).resolves.toEqual({ error: "Acknowledgement not admitted" });
    ...
    await expect(unavailable.json()).resolves.toEqual({ error: "Acknowledgement unavailable" });
  });
});
