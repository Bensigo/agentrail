...
function buildColumns(workspaceId: string): ColumnDef<RunRecord, unknown>[] {
  return [
    ...
    columnHelper.accessor("id", {
      ...
      cell: (info) => (
        <a
          ...
          className="text-[var(--gray-12)] hover:text-[#ffe629] transition-colors"
        >
    ...
    columnHelper.accessor("tokens_saved", {
      ...
      cell: (info) => {
        ...
        return (
          <span className="text-[#1fd8a4]" title="Context retrieval + cache reads">
            {formatTokens(saved)}
          </span>
        );
      },
    }),
  ] as ColumnDef<RunRecord, unknown>[];
}
...
export function RunsTable({ workspaceId, repositories }: RunsTableProps) {
  ...
  const succeeded = data.filter((r) => r.status === "success").length;
  ...
  return (
    <div className="flex flex-col gap-4">
      {!loading && !error && (
        <StatHeader
          stats={[
            { label: "Running", value: running, color: "orange" },
            { label: "Passed", value: succeeded, color: "green" },
