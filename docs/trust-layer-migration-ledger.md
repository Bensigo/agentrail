...
| Dependency-proposal contract bridge | current migration slice | focused converter test. A dependency candidate proposal can now be expressed as the canonical Acceptance Contract: candidate scope, baseline, expected files, verification commands, stop conditions, and every missing evidence item become durable open questions. The legacy dependency-watch route still takes its separate approval-to-issue path, so this is reusable conversion only—not yet an Acceptance Record intake, builder handoff, or dependency execution flow. |
