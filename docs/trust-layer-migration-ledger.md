...
| Console navigation, Home, and connector trust-layer pivot | `240ee81a`, `be45aea9`, `4c855efe`, `db0fef6`, `7f21c5ea`, `d5c2a63a`, `52d9e454`, current bounded cleanup slice | Focused Dashboard and connector tests. Acceptance Records and Approvals are primary. Home’s current primary surface is `OnboardingBanner`, then a server-rendered, explicit-UTC 30-day Outcome Summary, then `AcceptanceEvidencePanel`. The summary reads only canonical current exact-head evidence reviews, their Jace verdicts, append-only human decisions, and pending review/decision work. It renders counts/zeros and a no-completed-review state honestly; it distinguishes Jace `not_proven` from a human `changes_requested`, and does not call legacy factory/review-event metrics. `AcceptanceEvidencePanel` loads the five most recent Record headers and presents record-detail links, repository, issue/PR attachment, lifecycle state, updated time, and an honest empty state. Legacy digest/health component files may remain present but unmounted; compatibility/history routes may remain URL-reachable, and neither is a primary Home surface. Evidence & context exposes only Wiki; `/review-gates` remains URL-reachable for compatibility/history but is not product navigation. The Connector page keeps GitHub as the repository/PR/task-provenance anchor and preserves optional MCP/context plus investigation evidence providers; the connector sheet no longer renders heartbeat controls, while backend compatibility remains untouched. No browser or live connector proof is claimed. |
| Durable Brief provenance and Brief/Acceptance transition visibility | `0b454670`, `76f62704`, `38bff08a`, `5e89ebcb` | A long-lived editable Brief may seed multiple Acceptance Records; each Record has at most one source Brief. Each transition captures a workspace-scoped immutable header-plus-typed-item snapshot and provenance; later Brief edits cannot rewrite any Contract, Context Pack, review evidence, or decision. Brief detail shows zero/one/many linked Records, and Record detail links back to its current Brief while distinguishing the immutable snapshot from current editable context. Focused DB schema/integration tests, focused Console route/component/page tests, and both package typechecks pass. The migration was not applied to a local database and no authenticated Console browser flow ran. The hosted chat surface still does not embed this editable Brief panel while a conversation shapes a contract. |
...
4. Remove the now-obsolete dependency approval/publisher callers after a
   ...
   from human outcomes.
   Add the narrow hosted-chat/Console interaction that lets a user open and
   edit the linked Brief while a task conversation is shaping a Contract; it
   must use this same provenance rather than create a channel-local Brief or
   mutate confirmed Contract/Pack history.
...
6. Continue the separate post-MVP trust-layer evaluation program defined in
   ...
   real corpus, persistence/report publisher, corpus run, or market-value
   result.
