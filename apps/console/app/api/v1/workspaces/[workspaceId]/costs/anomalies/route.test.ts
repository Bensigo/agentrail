...
beforeEach(() => {
  ...
  vi.mocked(getCostAnomalies).mockResolvedValue(sampleAnomalies);
});
...
describe("GET /api/v1/workspaces/[workspaceId]/costs/anomalies", () => {
  ...
  it("parses time_from/time_to ISO query params and forwards them", async () => {
  ...
  it("200 with empty array when ClickHouse query throws (no 500)", async () => {
    vi.mocked(getCostAnomalies).mockRejectedValue(new Error("CH down"));
    const res = await GET(req(), { params: params() });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.anomalies).toEqual([]);
