...
- **Rung 3 honesty (B1):** no preview URL → behavioral ACs return `not_testable` with the concrete reason through the existing QA/reviewer vocabularies; the posted review says which environment rung was reached. Never a silent skip.
