...
4. The callback verifies `state` **against the caller's own authenticated session** — not merely well-formed — before writing anything: `state` is a single-use random token minted server-side at step 1 (stored with a TTL and consumed atomically at redemption — the exact house pattern the connect-link flow already uses in `chat_identities.link_token`; deliberately NOT HMAC, which has no precedent in this codebase), bound to the specific workspace the signed-in user is a member of, and the callback re-checks that membership at redemption. This is the same class of cross-tenant-hijack risk this codebase's own reviews have caught before in adjacent flows (magic-link workspace binding); a client-suppliable or replayable `state` would let one workspace's install get bound to another. Only then is `installation_id` stored on the workspace (new column `workspaces.githubInstallationId`, following the existing flat-column convention on that table — e.g. `discordWebhookUrl`).
5. Correction from code-level recon: `runner/repos/route.ts` has **no repo-listing handler** — it is a single POST implementing repo *creation* plus the connect chain (repository row, connector self-configure, webhook, onboard enqueue). Nothing lists a user's repos anywhere today (repos are connected by name), so there is no listing migration. `GET /installation/repositories` is still used, but only as the §8 reachability check after repo creation.
...
| `apps/console/app/api/v1/runner/repos/route.ts` | repo creation + connect chain (repository row, connector config, webhook registration, onboard enqueue); creation splits per §2's `create_repo` row — org installations create via `POST /orgs/{org}/repos` with the installation token, personal accounts get the guided-link reply (`POST /user/repos` is structurally unavailable to Apps) |
...
**Second rider — the Python personal-token paths.** Two Python seams read the personal token today: (a) `agentrail/cli/commands/issue.py` reads `GITHUB_OAUTH_TOKEN`/`GITHUB_TOKEN` from env (documented in `deploy/.env.production.example` as the credential for Jace's `create_issue` → `agentrail issue create --connector github`), and (b) its Postgres fallback plus the heartbeat's GitHub polling both resolve through `agentrail/heartbeat/token_provider.py`'s `get_github_token(workspace_id, executor)` — a second, independent implementation of the accounts-table read. Left alone, issues filed through these paths keep exactly the personal attribution this spec exists to kill. In this arc: the hosted deployment stops setting a personal token in env, and `token_provider.get_github_token` becomes the single Python seam that mints installation tokens (reading `workspaces.github_installation_id` via its existing executor, signing the App JWT in Python) — fixing the CLI fallback and heartbeat polling in one place. Acceptance criterion: **no hosted code path posts to GitHub with a personal token.**
