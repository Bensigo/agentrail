...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ execution: { id: claimed.execution.id, verificationPlanId: claimed.execution.verificationPlanId }, plan: { criterionId: claimed.plan.criterionId, modality: claimed.plan.modality, environmentId: claimed.plan.environmentId, flow: claimed.plan.flow, expectedBehavior: claimed.plan.expectedBehavior }, pr: { repository: claimed.repositoryFullName, number: claimed.prNumber, headSha: claimed.headSha } });
}
