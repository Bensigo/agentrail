...
| Channel-bound Context Pack compilation admission | current migration slice | focused runner admission and Jace tool-policy tests; Jace build. A confirmed session-bound Intake can admit only its connected repository and confirmed contract to the existing bounded `execute` compiler worker. The tool has no builder, repository, source, or Pack input; it reports admission only, never a completed Pack or implementation. No deployed compiler run is proven. |
| Console navigation, Home, and connector trust-layer pivot | `240ee81a`, `be45aea9`, `4c855efe`, `db0fef6`, `7f21c5ea` | Focused Dashboard (13), sidebar (25), and connector (168) tests. Acceptance Records and Approvals are primary. Home loads the five most recent Record headers and presents an Acceptance evidence panel with record-detail links, repository, issue/PR attachment, lifecycle state, updated time, and an honest empty state; it no longer mounts legacy review-metrics or false-green panels. Evidence & context exposes only Wiki; `/review-gates` remains URL-reachable for compatibility/history but is not product navigation. The Connector page keeps GitHub as the repository/PR/task-provenance anchor and preserves optional MCP/context plus investigation evidence providers, but removes rendered Heartbeat/autonomous-loop framing and states that optional adapters do not replace criterion-specific exact-head proof. GitHub-sheet and Wiki status copy likewise no longer promise Issue-Queue factory execution, push/open-PR behavior, or factory ownership. Factory work/runs, autonomous economics, memory/brief/investigation, and failure pages remain URL-reachable but are deliberately not primary product navigation. No browser, live data, or upstream connector proof yet. |
| Copy-only landing trust-layer pivot | current migration slice | focused marketing craft/pricing-copy suites and local browser evidence. The existing landing structure now describes Jace as the acceptance/evidence layer around Codex, Claude Code, or another selected builder; it no longer presents legacy factory run totals as trust proof or says Jace itself ships a PR. The chat demonstration now shows task → Acceptance Contract goal/boundary/checkable criteria → human contract confirmation → bounded Context Pack for the selected external builder, rather than a model estimate/approval/run-outcome sequence. Channel, closing CTA, and pricing copy use the same distinction. Browser inspection at `http://localhost:3000` exercised the demo confirmation state and captured its post-confirmation text; the dev process logs Auth.js `MissingSecret`, so authenticated Console flow is still unverified. The shared pricing-card capacity and price packaging remain a separate commercial decision; this copy slice does not claim a live external-builder or channel round-trip. |
...
| Criterion execution queue, guarded result seam, and opt-in Eve worker | `ee6f36d7` through `ec9bfc08`, `fbef412f`, current safe-preview queue slice | Focused runner admission/completion, artifact, plan, prompt, worker-core, worker-runtime, console-client, instrumentation, and DB proof-modality tests; DB query typecheck. The worker claims only plan-bound exact-head UI/API jobs whose matching preview is `ready` with a URL. A `pending`, `claimed`, or `booting` preview stays queued rather than becoming a fabricated `not_testable`; superseded, missing, mismatched, failed, or torn-down previews become explicitly `not_testable`, and a lost post-claim projection is terminalized. A no-id claim re-runs the same terminal-preview reconciliation, closing the narrow pre-claim race without treating a transient boot as terminal. A `proven` result must use artifacts from that same plan with the planned proof type: PNG/JPEG for UI and JSON for API. Until dedicated safe executors and artifact types exist, the route and DB query both refuse a planned `job` or `data` modality, and the proof seam refuses to reinterpret either as UI. Such criteria must be recorded as `not_testable` with a reason. It is default-off and has no live safe-preview/browser proof. |
