...
import {
  ...
  buildJudgmentConstraintsCheckUrl,
  checkJudgmentConstraints,
  resolveJudgmentConstraintsMode,
...
// ---------------------------------------------------------------------------
// Arc E2 — judgment constraints are checked before issue creation. The mode is
// explicit: off skips the preflight, warn permits creation, block refuses before
// the CLI can create a GitHub issue.
// ---------------------------------------------------------------------------

test("resolveJudgmentConstraintsMode defaults to off and accepts warn/block only", () => {
  assert.equal(resolveJudgmentConstraintsMode({}), "off");
  assert.equal(resolveJudgmentConstraintsMode({ AGENTRAIL_JUDGMENT_CONSTRAINTS_MODE: "warn" }), "warn");
  assert.equal(resolveJudgmentConstraintsMode({ AGENTRAIL_JUDGMENT_CONSTRAINTS_MODE: "BLOCK" }), "block");
  assert.equal(resolveJudgmentConstraintsMode({ AGENTRAIL_JUDGMENT_CONSTRAINTS_MODE: "fail-closed" }), "off");
});

test("buildJudgmentConstraintsCheckUrl joins the console base and check path", () => {
  assert.equal(
    buildJudgmentConstraintsCheckUrl("https://console.example.com"),
    "https://console.example.com/api/v1/runner/judgment-constraints/check",
  );
});

test("checkJudgmentConstraints: off mode skips transport and allows", async () => {
  let called = false;
  const result = await checkJudgmentConstraints({
    eveSessionId: "eve-1",
    env: { AGENTRAIL_JUDGMENT_CONSTRAINTS_MODE: "off" },
    issue: { title: "t" },
    transport: async () => {
      called = true;
  ...
  assert.deepEqual(result, { allow: true, mode: "off", violations: [] });
  assert.equal(called, false);
});

test("checkJudgmentConstraints: warn mode preserves violations but allows", async () => {
  const transport = fakeTransport(async () => ({
    status: 200,
    json: async () => ({
      allow: true,
      violations: [{ kind: "decision", id: "m1", reason: "violates decision", source: "memory_items" }],
    }),
  }));
  const result = await checkJudgmentConstraints({
    eveSessionId: "eve-1",
    env: {
      ...
      AGENTRAIL_JUDGMENT_CONSTRAINTS_MODE: "warn",
    },
    issue: { title: "t" },
    transport,
  });
...
test("runCreateIssue: block-mode judgment violation returns blocked and never runs the CLI", async () => {
  let execCalled = false;
  const transport = fakeTransport(async () => ({
    status: 200,
    json: async () => ({
      allow: false,
      message: "Recorded decision says do not use MongoDB.",
      violations: [{ kind: "decision", id: "m1", reason: "Do not use MongoDB.", source: "memory_items" }],
    }),
  }));
  const result = await runCreateIssue({
    execFileFn: async () => {
      execCalled = true;
      return { stdout: SUCCESS_STDOUT };
    },
    env: {
      JACE_CONSOLE_BASE_URL: "https://console.example.com",
      JACE_CONSOLE_TOKEN: "tok",
      JACE_TARGET_REPO: "Bensigo/agentrail",
      AGENTRAIL_JUDGMENT_CONSTRAINTS_MODE: "block",
    },
    title: "Switch storage to MongoDB",
    acceptanceCriteria: ["MongoDB stores issues"],
    eveSessionId: "eve-session-1",
    judgmentConstraintsTransport: transport,
  });
  assert.equal(execCalled, false);
  assert.equal(result.blocked, true);
  assert.equal(result.message, "Recorded decision says do not use MongoDB.");
  assert.equal(result.violations[0].id, "m1");
});

test("runCreateIssue: block-mode malformed judgment response fails closed before CLI", async () => {
  let execCalled = false;
  const transport = fakeTransport(async () => ({ status: 200, json: async () => ({ allow: false }) }));
  const result = await runCreateIssue({
    execFileFn: async () => {
      execCalled = true;
      return { stdout: SUCCESS_STDOUT };
  ...
  assert.deepEqual(result, {
    blocked: true,
    message: JUDGMENT_CONSTRAINTS_BLOCKED_MESSAGE,
    violations: [],
  });
});

test("runCreateIssue: warn-mode judgment violation still creates the issue", async () => {
  let execCalled = false;
  const transport = fakeTransport(async () => ({
    status: 200,
    json: async () => ({
      allow: true,
      violations: [{ kind: "rejected_approach", id: "j1", reason: "approach was rejected" }],
    }),
  }));
  const result = await runCreateIssue({
    execFileFn: async () => {
  ...
  assert.equal(result.number, 1042);
});
