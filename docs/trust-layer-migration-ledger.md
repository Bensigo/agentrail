...
| Review-bound redacted API artifact storage | current migration slice | focused Console redaction and API-artifact route tests, DB migration-journal coverage, and DB typecheck. The route accepts a structured request, response status, and assertions only for a current planned API criterion on its exact PR revision; it recursively redacts credential-bearing fields and common credential text before storing JSON, then records its digest. It does not execute an API call, verify an assertion, or declare a pass. |
