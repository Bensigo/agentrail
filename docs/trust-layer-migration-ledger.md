...
Last reconciled: 2026-08-07. Canonical product decision: [ADR 0012](adr/0012-jace-owns-the-acceptance-spine.md). Canonical post-MVP proof plan: [Jace trust-layer evaluation](prd/jace-trust-layer-evaluation.md).
...
retained diff. It does not repair invalid output into a pass. Persisted
exact-head plan/execution/artifact metadata is now attached to the worker claim
as bounded runtime evidence. At completion, Console discards reviewer-supplied
runtime evidence and re-derives it from that exact claim: a user-visible
criterion can be `proven` only from a persisted `proven` execution with its
selected artifact. The Jace worker now has an opt-in model adapter and startup
hook; deployment activation plus one authenticated exact-head claim, evidence
fetch, completion, and persisted verdict remain unverified.
...
| Automatic correction queue, builder-task inbox, durable Jace fallback, native MCP read/ack, and GitHub fallback dispatch | `7d560fcd` through `2890610f`, `f8789d6e`, `71079517`, `2438835f` | focused review-completion/queue/ack/inbox/dispatch tests, native MCP protocol test, MCP build/typecheck, and DB typecheck. A blocking review queues the exact correction for its unique PR-attached handoff when one exists; packets retain exact review revision and runtime evidence. Without a recorded builder handoff, it instead queues a target-validated `jace_task_inbox` packet on the same Acceptance Record. The inbox is durable and inspectable in the Record, but stays `queued`: it does not invent a recipient, acknowledgement, repair, or resume claim. A builder can retrieve its recorded task's MCP packet and acknowledge it through scoped MCP tools. The MCP acknowledgement now sends the required recorded builder/task-context identity as well as the delivery ID, so it reaches the server's fail-closed acknowledgement seam instead of making an incomplete request. A disposable local run created a current exact-head failed review through the real Console completion route, then drove the built MCP server to read its evidence-bound packet and explicitly acknowledge it; Console persisted `acknowledged` with a receipt timestamp. The run also exposed and `2438835f` fixed the source-only contracts export that had prevented the completion route from compiling. This is local protocol evidence, not a live Codex/Claude receipt, repair, or resumed task. GitHub dispatch posts a COMMENT-only PR issue-comment only for the current exact head and records delivered/failed. Neither carrier proves a live builder was notified or resumed. |
...
| Task-scoped external-builder Context Pack handoff | `fd297f13`, `c2e395fd`, `9252682a` | Focused Console detail/handoff and MCP builder-task route tests; 34 schema tests; and 9 real-local-Postgres Change Record integrations. An owner/admin selects an active, workspace-scoped agent-MCP credential with both `acceptance:read` and `acceptance:correction:ack` when recording a builder/task/branch/confirmed-Contract/compiled-Pack handoff. The credential ID is persisted in the handoff and event provenance. The MCP builder-task route supplies its authenticated credential ID; the database requires it to match the selected, non-revoked agent-MCP credential before exposing the bounded Contract/Pack or recording its delivery. A legacy null binding, foreign/wrong/revoked credential, malformed credential ID, stale exact head, or missing delivery fails closed. The owner UI offers only eligible active credentials and will not submit without one. This is source, route, and local-Postgres evidence only. Dia renders the authenticated Console but this local workspace has no Change Record, so the handoff form has not been rendered or submitted in a real browser; no live Codex/Claude session, external implementation, or correction receipt is claimed. |
...
| Dashboard outcome range runtime check | current migration slice | Dia on the authenticated local Console at `localhost:3001` rendered the trust-outcome cards and honest empty state. Selecting `24h` produced the canonical `?range=24h` route and changed the displayed exact-head UTC window to one day. This proves local rendering and range navigation only; no non-zero review/human-decision data, hosted deployment, or connector flow was exercised. |
...
| Exact-review final human PR decision | `f2f9492d` plus current migration slice | Focused Console route/detail and DB decision-validator tests; a real local Postgres integration creates a failed exact-head review, rejects ordinary approval, records one `changes_requested` decision, and proves retry returns the original immutable event rather than relabelling Jace's verdict. The event-row mapper now accepts both raw SQL and Drizzle row shapes, so idempotent API retries return the stored decision payload. Record detail consumes only a valid current-review decision event, shows its recorded decision/rationale, and removes further decision controls; malformed, foreign, wrong-review, or superseded-review events do not affect controls. Owners/admins can append one immutable `approved`, `changes_requested`, `rejected`, or explicit `approved_with_exception` decision only for a current exact-head Evidence Review. A standard approval is refused unless Jace recorded `proven`; an exception requires a rationale and does not alter Jace's independent verdict. This records no GitHub merge. Dia renders the authenticated local Changes zero state, but no synthetic/real final decision was rendered or submitted in the browser; deployed/migrated-production human-decision proof remains unverified. The older `review_events` rework/revert ledger remains aggregate outcome infrastructure only; it is not used as the acceptance decision source. |
| Server-resolved runtime evidence at review completion | current migration slice | Focused DB resolver integration, Console claim/completion route tests, Jace evaluator tests, and Console/DB typechecks. The review claim carries only bounded metadata from plans, executions, and selected stored artifacts bound to the same Record, confirmed Contract version, current PR attachment/revision, exact head, and workspace. The evaluator may see that metadata, but completion overwrites every submitted criterion's runtime evidence with the canonical claim-derived values. A model/worker cannot fabricate an artifact, environment, observed result, or a `proven` user-visible criterion; a non-proven/missing persisted execution remains `not_proven`. The DB integration proves an exact local API plan/execution/artifact resolves into a claimed review. This is local source/database/route evidence only: no deployed PR preview, live model completion, or human final decision is proven. |
| Credential-bound, current-head MCP correction inbox | `38d56222`, `9252682a` | Focused MCP read/ack route tests and a real local Postgres integration. A correction is readable or acknowledgeable only through an `mcp_task_context` delivery whose exact builder/task handoff has a recorded MCP Context Pack delivery for the same human-selected MCP credential. Both queries require the correction's exact PR revision to remain current. A different workspace-scoped credential receives no packet and cannot acknowledge it; a malformed credential fails closed without querying Postgres, and a superseded head is neither readable nor acknowledgeable. This remains a pull inbox, not a push into a live Codex/Claude session; acknowledgement proves only recorded receipt, never repair, merge, or runtime proof. No live MCP client or deployed PR revision has run this path. |
...
| Criterion execution queue, guarded result seam, and opt-in exact-plan worker | `ee6f36d7` through `ec9bfc08`, `fbef412f`, `0a615e7f`, `b146664c`, `73c492f4`, `2d27f040`, `8d49628c`, `945810c2` | Focused runner admission/completion, artifact, plan, worker-core/worker-runtime, API/browser executor, console-client, migration-schema, and DB proof-modality tests; DB query typecheck. The worker claims only plan-bound exact-head UI/API/data jobs whose matching preview is `ready` with a URL. A planned UI criterion persists a bounded immutable `uiSteps` list; a planned API criterion persists only `GET` + same-origin path + expected status. UI uses only those persisted browser actions through a private sidecar, captures a same-origin PNG/JPEG, and uploads it through the plan-bound route. API uses no root-Jace/QA model turn: it performs only the persisted same-origin `GET`, omits credentials, rejects redirects, never reads the response body, compares only the planned status, and uploads a redacted plan-bound request/status/assertion card. A `proven` result requires an artifact from that exact plan with the planned proof type: PNG/JPEG for UI and JSON for API/data. Unsafe/missing descriptors and previews are `not_testable`; a safe request failure, redirect, status mismatch, or storage failure is `not_proven`. The API executor now rejects dot/traversal request-path segments. A local-only loopback harness drives the injected API executor and worker claim/complete seam in memory, proving a same-origin GET and redacted evidence card without Console/GitHub/model/DB transport; its injected fetch avoids ambient external requests. A separate live-local Postgres integration test creates and cascade-cleans a synthetic workspace/Contract/exact PR revision/ready preview/API plan/execution, verifies claim plus a JSON-artifact `proven` completion, rejects a wrong worker and unready preview, and caught/fixed the raw-SQL `uuid = text` preview join by comparing the UUID preview ID as text. A further disposable local run calls the real authenticated Console claim/complete HTTP routes and the actual Jace worker against a loopback exact-preview API flow. With artifact storage absent, the worker gets the planned `200` then terminally persists `not_proven`; `8d49628c` omits absent optional result fields so that fail-closed completion passes Console validation rather than stranding a claim. This proves local auth, route, Postgres, worker, and storage-failure behavior only. Job plans remain explicitly `not_testable` pending a bounded trigger/output executor. Production Compose contains private browser-sidecar wiring and worker activation remains an explicit Console-token/artifact-store opt-in. No configured artifact store, deployed safe preview, browser UI flow, external builder, or human decision has run live. |
| Bounded data readback execution | `a75aa0a4`, `945810c2` | Data plans now use a distinct restricted descriptor: same-preview-origin `GET`, exact status, and 1–12 JSON-pointer scalar equality assertions. The worker reuses the exact-plan API transport with credentials omitted and redirects rejected; it uploads a redacted, plan-bound JSON evidence card only after the assertions hold. Unsafe descriptors/previews are `not_testable`; failed assertions, fetches, redirects, status checks, or artifact upload are `not_proven`. Focused Console plan/artifact tests, DB typecheck, and Jace executor/worker tests pass. The opt-in local integration now seeds a synthetic data plan and exact ready head, calls the real Console claim/complete routes plus Jace worker, reads the stored MinIO JSON card back, verifies the exact `GET`, status, and `/ok = true` assertion, and cleans up. The claim route also returns the persisted data descriptor; this prevents a claim from silently losing the assertions. This is direct local integration evidence only, not a deployed safe preview or customer criterion. Job criteria remain explicitly `not_testable` until a bounded trigger/output executor exists. |

| Execution-worker status correction | `a75aa0a4` | The earlier worker row predates data-readback support: its data-refusal statement is superseded by the bounded data slice; its job `not_testable` statement remains current. |
| Local exact-head API artifact runtime proof | `01a4375d` | An explicit opt-in integration test seeds a synthetic confirmed Contract, exact PR head, ready loopback preview, planned API criterion, and queued execution. It calls the real authenticated local Console claim, API-artifact, and completion routes through the real Jace worker; MinIO stores the plan-bound JSON evidence card, the test reads it back to verify its exact GET/status/assertion, then deletes the object and cascades the synthetic workspace rows. The run also exposed migration-journal/schema drift, repaired by forward migration `0102_evidence_verification_data_request_repair.sql`; local migration and schema-constraint presence were verified. This is direct local integration evidence only: its loopback preview is not a deployed PR environment, and it proves neither UI, data, job, builder, nor channel behavior. |
| Local exact-head UI screenshot runtime proof | `6e61a1df` | A second explicit opt-in integration case seeds a synthetic confirmed Contract, exact PR head, ready local preview, and persisted `open` → `expect_text` → `screenshot` UI steps. The real Dockerized agent-browser MCP sidecar executes only those steps, Console stores a PNG through the plan-bound artifact route, the test reads back the PNG signature from MinIO, then removes the object and cascades synthetic rows. The test exposes the preview host explicitly because a Docker sidecar cannot reach host-only `127.0.0.1`; local Docker uses `host.docker.internal`. The browser executor now bounds sidecar `close()` at two seconds so a transport cleanup hang cannot strand a completed claim. This is direct local integration evidence only: it is not a deployed PR preview or a real customer criterion. |
...
safe UI and API flows and bind their observed results to these artifacts.
Review completion now consumes only that persisted exact-head evidence rather
than model-generated proof claims. Delivery is currently queue plus
...
1. The local API, UI, and data workers have completed artifact-backed loopback
   proofs. Repeat them on a deployed safe exact-head PR environment. Add a job trigger/output
   executor only when a safe bounded job surface exists, otherwise retain
   `not_testable` rather than forcing those modalities through screenshots.
2. Prove a live supported external-builder delivery path and resume semantics.
   ...
   first builder-task read and correction inbox now require the one
   human-selected active agent-MCP credential persisted on the handoff. Prove
   that binding with a real live builder session and retain its delivery/receipt
   evidence; do not treat local protocol success or a delivery audit as
   implementation, notification, or task resumption.
