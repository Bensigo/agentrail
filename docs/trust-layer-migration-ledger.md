...
| Console navigation and Home trust-layer pivot | `240ee81a`, `be45aea9`, `4c855efe` | Focused Dashboard (13) and sidebar (25) tests. Acceptance Records and Approvals are primary. Home loads the five most recent Record headers and presents an Acceptance evidence panel with record-detail links, repository, issue/PR attachment, lifecycle state, updated time, and an honest empty state; it no longer mounts legacy review-metrics or false-green panels. Evidence & context exposes only Wiki; `/review-gates` remains URL-reachable for compatibility/history but is not product navigation. Factory work/runs, autonomous economics, memory/brief/investigation, and failure pages remain URL-reachable but are deliberately not primary product navigation. The connector catalog remains a separate unresolved product-scope slice. No browser or live data proof yet. |
