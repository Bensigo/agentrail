"""Secret / prod-push guardrail (M037, issue #773).

A deep module (pure logic, no I/O) that blocks two classes of dangerous
push/commit and records every block as an **Audit Event**:

- a commit/push whose staged/committed content contains a **detected secret**
  (AC1);
- a push to a **protected / production target** (AC2).

Per ``docs/design/verification-contract-architecture.md`` the decision logic
here is pure and deterministic; the network/git I/O lives at the edge
(``emit_block_audit_event`` follows the existing ``failure_push`` /
``activity_push`` POST pattern). ``evaluate_push`` returns a plain
``PushDecision`` so callers can act on it without touching this module's
internals.
"""
...
# ---------------------------------------------------------------------------
# Audit Event (edge): every block records a sensitive-action / policy decision
# ---------------------------------------------------------------------------
