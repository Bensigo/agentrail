...
def _case(tmp_path: Path):
    payload = {
        ...
        "sourceConversation": [{"id": "m1", "text": "private history"}],
        "pinned": {"repo": "acme/app", "commit": "abc"},
        "relevantSources": ["src/private.ts:1-2"],
        "approvedContract": {
            "version": "contract-v1",
...
def test_builder_inputs_expose_only_the_material_for_each_canonical_arm(
    tmp_path: Path,
) -> None:
    case = _case(tmp_path)
    ...
    assert set(alone.__dataclass_fields__) == {
        "arm",
        "user_request",
        "contract",
        "context_pack",
...
def test_builder_contract_is_a_frozen_copy_and_unknown_arm_is_refused(
    tmp_path: Path,
) -> None:
    case = _case(tmp_path)
...
def test_lineage_binds_exact_case_contract_pack_head_and_environment(
    tmp_path: Path,
) -> None:
    case = _case(tmp_path)
...
def test_lineage_refuses_non_frozen_or_ambiguous_bindings(
    tmp_path: Path,
    kwargs: dict,
    message: str,
) -> None:
...
def test_lineage_fails_closed_if_ambiguous_bindings_reach_it(tmp_path: Path) -> None:
    case = _case(tmp_path)
    case.pr_revisions.append(
        {"headSha": "deadbeef", "diffIdentity": "sha256:other"}
    )
    with pytest.raises(ValueError, match="PR head is ambiguous"):
        acceptance_lineage(
            case,
            "full-jace-loop",
            pr_head="deadbeef",
    ...
    case.pr_revisions.pop()
    case.pr_revisions.append(
        {"headSha": "other-head", "diffIdentity": "sha256:diff"}
    )
    with pytest.raises(ValueError, match="diff identity is ambiguous"):
        acceptance_lineage(
            case,
            "full-jace-loop",
            pr_head="deadbeef",
    ...
    case.pr_revisions.pop()
    case.environments.append({"id": "preview-1", "modality": "api"})
    with pytest.raises(ValueError, match="environment is ambiguous"):
        acceptance_lineage(
            case,
            "full-jace-loop",
            pr_head="deadbeef",
