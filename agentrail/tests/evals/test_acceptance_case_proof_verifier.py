...
def _case(tmp_path: Path, *, modality: str = "ui", verdict: str = "proven"):
    payload = {
        "name": "criterion-proof", "split": "held-out", "corpusVersion": "corpus-v1",
        "userRequest": "prove one criterion", "sourceConversation": [{"id": "m1", "text": "request"}],
        "pinned": {"repo": "acme/app", "commit": "base"}, "relevantSources": ["src/save.ts:1-2"],
        "approvedContract": {"version": "contract-v1", "acceptanceCriteria": [{"id": "criterion"}]},
        ...
        "independentLabels": {
            "contract": {}, "context": {}, "review": {}, "correction": {}, "outcome": {},
            "proof": {"criteria": [{"criterionId": "criterion", "modality": modality, "expectedVerdict": verdict, "expectedStatus": 200}]},
        },
        "source": {"issue": 1},
    }
...
def test_accepts_exact_head_ui_proof_with_inspectable_screenshot(tmp_path: Path) -> None:
    ...
    result = verify_criterion_proof(case, lineage, CriterionProofClaim(
        "criterion", "ui", "proven", "deadbeef", "preview-1", "Save confirmation is visible",
...
def test_rejects_stale_or_generic_ui_proof(tmp_path: Path) -> None:
    case, lineage = _case(tmp_path)
    stale = verify_criterion_proof(case, lineage, CriterionProofClaim(
        "criterion", "ui", "proven", "other", "preview-1", "Save confirmation is visible",
        (ProofArtifact("artifact://save", "image/png"),),
    ))
    smoke = verify_criterion_proof(case, lineage, CriterionProofClaim(
        "criterion", "ui", "proven", "deadbeef", "preview-1", "",
...
def test_requires_redacted_exact_status_api_evidence(tmp_path: Path) -> None:
    ...
    invalid = verify_criterion_proof(case, lineage, CriterionProofClaim(
        "criterion", "api", "proven", "deadbeef", "preview-1", "API returned 200",
        (ProofArtifact("artifact://api", "application/json", redacted=False),), response_status=200,
    ))
    valid = verify_criterion_proof(case, lineage, CriterionProofClaim(
        "criterion", "api", "proven", "deadbeef", "preview-1", "API returned 200",
        (ProofArtifact("artifact://api", "application/json", redacted=True),), response_status=200,
    ))
    assert invalid.status == "invalid" and "redacted JSON" in invalid.reason
    assert valid.valid


def test_credits_independently_labelled_not_testable_without_artifacts(tmp_path: Path) -> None:
    case, lineage = _case(tmp_path, verdict="not_testable")
    result = verify_criterion_proof(case, lineage, CriterionProofClaim("criterion", "ui", "not_testable", "deadbeef", "preview-1"))
