...
// evidence, `search_events` a secondary source; `fetch_signals` is here to
// CONFIRM a candidate's impact — a metric that actually moved right after a
// deploy is stronger evidence than the deploy timestamp alone.
//
...
export default defineTool({
  description:
    ...
    "throughput — across every credentialed signals provider for this workspace — useful here to confirm a " +
    "change candidate's impact (did the metric actually move right after the deploy?). windowStart/windowEnd " +
    "(ISO-8601, both REQUIRED — every evidence query is scoped to a window) bound the search; scope " +
    "optionally narrows within a provider (e.g. a service name); query optionally narrows which metric/signal " +
    "comes back; limit caps how much comes back before the envelope's own size caps. " +
    "\n\n" +
