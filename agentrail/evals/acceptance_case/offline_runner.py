...
This is an evaluator boundary, not an agent runner. The builder adapter
receives only the arm-specific input plus an operational checkout target; it
returns the PR head/environment it produced. The independent scorer then
receives the frozen case and can consult hidden labels.
"""
    ...
    return AcceptanceObservation(
        ...
        provenance={
            ...
            "contextPackTokenBudget": (
                str(lineage.context_pack_token_budget) if includes_pack else "none"
            ),
