...
1. **Precision numerator — high-value token share.** An item counts as required/anchor (high-value, not filler) when `sourceType ∈ {context_doc, taste_doc}` **OR** `authority ∈ {critical, high}`. Broadened from `{critical}`-only so the metric reads as "share of budget on high-value context vs. filler" across code-heavy packs (which carry no context docs and would otherwise always report `0`), rather than strictly "context-doc budget share." `precision_at_budget = sum(tokenEstimate of high-value selected items) ÷ selected_context_tokens`, clamped to `[0,1]`; `0.0` when `selected_context_tokens <= 0`. (No score threshold — deterministic.)
