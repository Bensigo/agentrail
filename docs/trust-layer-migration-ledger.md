...
| Credential-bound, current-head MCP correction inbox | current migration slice | Focused MCP read/ack route tests and a real local Postgres integration. A correction is readable or acknowledgeable only through an `mcp_task_context` delivery whose exact builder/task handoff has a recorded MCP Context Pack delivery for the same MCP credential. Both queries require the correction's exact PR revision to remain current. A different workspace-scoped credential receives no packet and cannot acknowledge it; a superseded head is neither readable nor acknowledgeable. This remains a pull inbox, not a push into a live Codex/Claude session; acknowledgement proves only recorded receipt, never repair, merge, or runtime proof. No live MCP client or deployed PR revision has run this path. |
