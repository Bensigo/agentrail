...
// signatures and timing; `fetch_changes` (this file's sibling) is a
// secondary source — a deviation's timing often only makes sense next to
...
export default defineTool({
  description:
    ...
    "timeline) — your primary way to find WHERE and WHEN the system deviates from baseline: error bursts, " +
    "latency/saturation signatures, new log patterns. windowStart/windowEnd (ISO-8601, both REQUIRED — every " +
    "evidence query is scoped to a window) bound the search; query is what you're searching FOR — pass it, " +
    "this verb is far less useful without one; scope optionally narrows within a provider; limit caps how " +
    "much comes back before the envelope's own size caps. " +
    "\n\n" +
