...
function req(body: unknown = {}): NextRequest {
  return new NextRequest(
    `http://localhost/api/v1/workspaces/${WS}/onboarding/skip-invite-team`,
    {
      method: "POST",
...
describe("POST /api/v1/workspaces/[workspaceId]/onboarding/skip-invite-team", () => {
  ...
  it("records the skip on the github connector row (piggybacked, default body)", async () => {
    const res = await POST(req(), params());
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.skipped).toBe(true);
    expect(upsertConnector).toHaveBeenCalledWith(WS, "github", {
      config: { inviteTeamSkippedAt: expect.any(String) },
    });
  });
  ...
  it("clears the skip when skip:false is passed (undo)", async () => {
    ...
    expect(upsertConnector).toHaveBeenCalledWith(WS, "github", {
      config: { inviteTeamSkippedAt: undefined },
    });
  });
});
