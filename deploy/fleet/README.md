...
4. **Set environment variables**:
   ...
   | `AGENTRAIL_LANGFUSE_ENABLED` + `LANGFUSE_HOST` + `LANGFUSE_PUBLIC_KEY` + `LANGFUSE_SECRET_KEY` | Recommended | Turns on the per-run Langfuse tracer (`RunTracer.start()` in `agentrail/run/pipeline.py`). **All four are required together** — `agentrail/observability/langfuse_client.py` gates on the flag AND builds its client from the three keys, so any one missing leaves the tracer INERT BY CONSTRUCTION: runs execute normally and emit no trace, no per-phase generation, and no cost, with no error anywhere. That silence is the whole footgun — a fleet with these unset looks perfectly healthy while every execution run is unobserved (prod ran this way until 2026-07-26; only Jace's chat traces were landing, because the `jace` service had its own keys and this one had none). Point them at the SAME Langfuse project the `jace` service uses, so chat traces and the runs they spawn share one project — and mind the region: the host is a per-project property (`https://jp.cloud.langfuse.com` ≠ `https://cloud.langfuse.com`), and keys from one region simply read as empty against another. `LANGFUSE_BASE_URL` is accepted as a fallback for `LANGFUSE_HOST`; setting both is harmless and matches the TS side, which reads only `LANGFUSE_BASE_URL`. |
