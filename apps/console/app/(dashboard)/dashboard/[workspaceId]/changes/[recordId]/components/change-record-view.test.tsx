...
describe("Change Record detail view", () => {
  ...
  it("strictly validates Go 1.26 receipts and the immutable external-builder Pack", () => {
    expect(isDependencyObservationsEnvelope(goDependencyEnvelope())).toBe(true);
    expect(isDependencyObservationsEnvelope(
      goDependencyEnvelope(false, "github.com/acme/widget/v2", "v2.1.0", "v2.2.0")
    )).toBe(true);
    expect(isDependencyObservationsEnvelope(
      goDependencyEnvelope(false, "gopkg.in/yaml.v3", "v3.0.0", "v3.0.1")
    )).toBe(true);

    const goWithPack = goDependencyEnvelope(true);
    const rendered = DependencyObservationsPanel({
      dependencyObservations: goWithPack,
      canApproveDependencyObservation: true,
      onApprove: () => undefined,
      approvingObservationEventId: null,
      approvalError: null,
    ...
    const content = textContent(rendered);
    expect(content).toContain("github.com/acme/widget v1.2.3 → v1.3.0");
    expect(content).toContain("safe · go / go-modules · 1.26.1");
    expect(content).toContain("safe · go 1.26.1 · go_1_26_root_mod_sum_public_proxy_v1");
    ...
    for (const mutate of [
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/Acme/widget";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = `${"a".repeat(64)}.com/acme/widget`;
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = `example.com/${"a".repeat(204)}`;
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/acme/con.txt";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/acme/foo~1";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/acme/foo~1.bar";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "con.example/acme/widget";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.dependencyKind = "devDependencies";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.specifier = "^v1.2.3";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.targetVersion = "v2.0.0";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        Object.assign(value.observations[0]!.observation.candidate, {
          package: "gopkg.in/yaml/v3",
          currentVersion: "v3.0.0", specifier: "v3.0.0", targetVersion: "v3.0.1",
        });
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/acme/widget/v1";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "gopkg.in/yaml";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "gopkg.in/yaml.v1";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/acme/widget/v01";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/acme/widget/v1.2";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.candidate.package = "github.com/acme/widget/v2.0";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.runtime.version = "1.25.9";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.packageManager.name = "go-modules";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.packageManager.version = "1.27.0";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.packageManager.updateArgv.push("-u");
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.manifest.path = "services/api/go.mod";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.lockfile.path = "services/api/go.sum";
      },
      (value: ReturnType<typeof goDependencyEnvelope>) => {
        value.observations[0]!.observation.security.reference =
          "osv:Go:github.com/acme/widget@v1.3.0";
      },
    ]) {
      const invalid = goDependencyEnvelope();
      mutate(invalid);
      expect(isDependencyObservationsEnvelope(invalid)).toBe(false);
    }
  });

  it("keeps a frozen unsupported Go refusal readable without promoting it", () => {
    const historical = goDependencyEnvelope(
      false, "gopkg.in/yaml/v3", "v3.0.0", "v3.0.1"
    );
    const observation = historical.observations[0]!.observation;
    observation.status = "refused_unsupported_profile";
    observation.reasons = ["unsupported_manager_profile"];
  ...
  it("does not admit Cargo as a canonical current receipt profile", () => {
    const cargo = goDependencyEnvelope();
    const identity = {
      ecosystem: "rust", manager: "cargo", profile: "cargo_lockfile_only_v1",
    };
    cargo.observations[0]!.observation.candidate.identity = identity;
    cargo.observations[0]!.observation.runtime.identity = identity;
    cargo.observations[0]!.observation.security.identity = identity;
    expect(isDependencyObservationsEnvelope(cargo)).toBe(false);
  });
