...
test("MCP intake, task-context messages, builder handoff, and correction delivery use bounded identity", async () => {
  ...
  try {
    ...
    send({ jsonrpc: "2.0", id: 7, method: "tools/call", params: { name: "correction_delivery_acknowledge", arguments: { deliveryId: "22222222-2222-4222-8222-222222222222", builder: "codex", taskContextKey: "task / a&b", detail: "Packet read" } } });
  ...
  assert.equal(apiRequests.find((request) => request.url?.endsWith("/ack"))?.body, JSON.stringify({ builder: "codex", taskContextKey: "task / a&b", detail: "Packet read" }));
});
