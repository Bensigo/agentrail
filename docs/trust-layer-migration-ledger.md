...
| Criterion execution queue, guarded result seam, and opt-in Eve worker | `ee6f36d7` through `ec9bfc08`, `fbef412f`, current safe-preview queue slice | Focused runner admission/completion, artifact, plan, prompt, worker-core, worker-runtime, console-client, migration-schema, and DB proof-modality tests; DB query typecheck. The worker claims only plan-bound exact-head UI/API jobs whose matching preview is `ready` with a URL. A planned UI criterion now persists a bounded, immutable `uiSteps` list (`open` relative path, `click`, `fill`, bounded key press, `expect_text`, and decisive `screenshot` only); Console admission rejects absent, unknown, external, oversized, extra-payload, or over-budget steps. The claim carries those exact actions to the worker. Historical UI plans without them terminate `not_testable` before Jace/QA is invoked. A `pending`, `claimed`, or `booting` preview stays queued rather than becoming a fabricated `not_testable`; superseded, missing, mismatched, failed, or torn-down previews become explicitly `not_testable`, and a lost post-claim projection is terminalized. A no-id claim re-runs the same terminal-preview reconciliation, closing the narrow pre-claim race without treating a transient boot as terminal. A `proven` result must use artifacts from that same plan with the planned proof type: PNG/JPEG for UI and JSON for API. Until dedicated safe executors and artifact types exist, the route and DB query both refuse a planned `job` or `data` modality, and the proof seam refuses to reinterpret either as UI. Such criteria must be recorded as `not_testable` with a reason. The current worker still asks Jace to dispatch the restricted steps; there is no deployed direct browser adapter or live safe-preview/browser proof. |
