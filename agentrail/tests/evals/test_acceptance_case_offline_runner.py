...
def _case(tmp_path: Path):
    payload = {
        ...
        "sourceConversation": [{"id": "m1", "text": "hidden chat"}],
...
class _Scorer:
    def score(self, case, lineage, attempt):
        ...
        return [
            ScoredAttempt("proof", "ui", True, True, "artifact-1"),
            ScoredAttempt(
                "proof",
                "ui-artifact-validity",
                True,
                True,
                "artifact-1",
            ),
...
def _provenance():
    return RunProvenance(
        ...
        outcome_source="hidden-label-v1",
    )


def _policy():
    floor = MetricFloor(1, 1, 0, 0)
...
def test_runs_all_four_arms_without_leaking_evaluator_only_case_data(
    tmp_path: Path,
) -> None:
    case = _case(tmp_path)
    executor = _Executor()
    ...
    assert set(executor.inputs[0][0].__dataclass_fields__) == {
        "arm",
        "user_request",
        "contract",
        "context_pack",
    ...
    assert executor.inputs[1][0].contract["version"] == "contract-v1"
    assert executor.inputs[1][0].context_pack is None
