...
// `fetch_traces` complements both by showing WHERE in the call chain a
// deviation originates — one exemplar request's actual path and duration
// through the system, not just that an error or a slowdown occurred
// somewhere.
//
...
export default defineTool({
  description:
    ...
    "duration and status — across every credentialed traces provider for this workspace — useful here for " +
    "pinning down WHERE in the call chain a deviation originates, complementing search_events' log/event " +
    ...
    "narrows which trace/span comes back (e.g. an operation name); limit caps how much comes back before the " +
    "envelope's own size caps. " +
    "\n\n" +
