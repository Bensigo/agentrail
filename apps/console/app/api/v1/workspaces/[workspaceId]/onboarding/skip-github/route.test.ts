...
function req(body: unknown = {}): NextRequest {
  return new NextRequest(
    `http://localhost/api/v1/workspaces/${WS}/onboarding/skip-github`,
    {
      method: "POST",
...
describe("POST /api/v1/workspaces/[workspaceId]/onboarding/skip-github", () => {
  ...
  it("records the skip on the github connector row (default body)", async () => {
    const res = await POST(req(), params());
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.skipped).toBe(true);
    expect(upsertConnector).toHaveBeenCalledWith(WS, "github", {
      config: { githubSkippedAt: expect.any(String) },
    });
  });
  ...
  it("clears the skip when skip:false is passed (undo)", async () => {
    ...
    expect(upsertConnector).toHaveBeenCalledWith(WS, "github", {
      config: { githubSkippedAt: undefined },
    });
  });
});
