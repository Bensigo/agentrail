...
// ---------------------------------------------------------------------------
// Savings aggregation (M025 cost-wedge dashboard)
// ---------------------------------------------------------------------------

/**
 * Representative pricing model for context-pack savings.
 * context_packs has no per-pack model column, so this rate is always applied.
 * Mirrors agentrail/context/pricing.py _FALLBACK_RATE (claude-sonnet-4-5 input rate).
 */
export const SAVINGS_REPRESENTATIVE_MODEL = "claude-sonnet-4-5";
/** $/Mtok — sonnet input rate; see agentrail/context/pricing.py PRICE_TABLE */
export const SAVINGS_RATE_PER_MTOK = 3.0;

export type SavingsAgentName = "claude" | "codex" | "cursor";

const KNOWN_AGENTS: SavingsAgentName[] = ["claude", "codex", "cursor"];

export interface SavingsTimeOptions {
  timeFrom?: Date;
  timeTo?: Date;
}

export interface WorkspaceSavingsResult {
  tokensSaved: number;
  dollarsSaved: number;
  model: string;
  ratePerMtok: number;
...
export interface AgentSavingsBreakdownRow {
  agent: SavingsAgentName | null;
  totalCostUsd: number;
  dollarsSaved: number;
  eventCount: number;
}
...
/**
 * Workspace-level tokens/dollars saved from context_packs.tokens_saved,
 * priced at the representative model rate (always an estimate because
 * context_packs carries no per-pack model column).
 */
export async function aggregateWorkspaceSavings(
  workspaceId: string,
  opts: SavingsTimeOptions = {},
  ch: QueryClient = client
): Promise<WorkspaceSavingsResult> {
  const conditions: string[] = ["workspace_id = {workspaceId: String}"];
  const queryParams: Record<string, unknown> = { workspaceId };
  ...
  const result = await ch.query({
    query: `
      SELECT sum(tokens_saved) AS tokens_saved
      FROM context_packs
      WHERE ${conditions.join(" AND ")}
    `,
    query_params: queryParams,
  ...
  const rows = await result.json<Record<string, unknown>>();
  const tokensSaved = Number(rows[0]?.tokens_saved ?? 0);

  return {
    tokensSaved,
    dollarsSaved: (tokensSaved / 1_000_000) * SAVINGS_RATE_PER_MTOK,
    model: SAVINGS_REPRESENTATIVE_MODEL,
    ratePerMtok: SAVINGS_RATE_PER_MTOK,
    estimateFlag: true,
  };
}

/**
 * Per-agent cost + savings breakdown.
 *
 * Cost side: cost_events bucketed by model prefix:
 *   claude-{x}  → claude
 *   gpt-{x}, codex-{x}, o[0-9]-{x}  → codex
 *   cursor-{x}  → cursor
 *
 * Savings side: context_packs.tokens_saved attributed to agents by joining
 * run_ids against run_events.agent. Tokens from runs with no matching
 * run_events row surface in a null-agent overflow row.
 *
 * Always returns explicit rows for claude, codex, and cursor even when
 * eventCount is zero (AC2). A null-agent overflow row is appended only when
 * there are unresolvable tokens.
 */
export async function getAgentSavingsBreakdown(
  workspaceId: string,
  opts: SavingsTimeOptions = {},
  ch: QueryClient = client
): Promise<AgentSavingsBreakdownRow[]> {
  const costConditions: string[] = ["workspace_id = {workspaceId: String}"];
  const packConditions: string[] = ["workspace_id = {workspaceId: String}"];
  const queryParams: Record<string, unknown> = { workspaceId };

  if (opts.timeFrom) {
    costConditions.push("occurred_at >= {timeFrom: DateTime64(3)}");
    packConditions.push("occurred_at >= {timeFrom: DateTime64(3)}");
    queryParams.timeFrom = formatClickHouseDateTime(opts.timeFrom);
  }
  if (opts.timeTo) {
    costConditions.push("occurred_at <= {timeTo: DateTime64(3)}");
    packConditions.push("occurred_at <= {timeTo: DateTime64(3)}");
    queryParams.timeTo = formatClickHouseDateTime(opts.timeTo);
  }

  // --- Cost side: aggregate cost_events by agent bucket ---
  const costResult = await ch.query({
    query: `
      SELECT
        multiIf(
  ...
  const costRows = await costResult.json<Record<string, unknown>>();

  // Build cost lookup keyed by known agent names only
  const costByAgent = new Map<string, { totalCostUsd: number; eventCount: number }>();
  for (const r of costRows) {
    const agent = String(r.agent ?? "");
    if (KNOWN_AGENTS.includes(agent as SavingsAgentName)) {
      costByAgent.set(agent, {
        totalCostUsd: Number(r.total_cost_usd ?? 0),
  ...
  // --- Savings side: tokens_saved per run_id from context_packs ---
  const packResult = await ch.query({
    query: `
      SELECT run_id, sum(tokens_saved) AS tokens_saved
      FROM context_packs
      WHERE ${packConditions.join(" AND ")}
      GROUP BY run_id
    `,
    query_params: queryParams,
    format: "JSONEachRow",
  });
  const packRows = await packResult.json<Record<string, unknown>>();

  const tokensSavedByRunId = new Map<string, number>();
  for (const r of packRows) {
    tokensSavedByRunId.set(String(r.run_id ?? ""), Number(r.tokens_saved ?? 0));
  }

  // --- Agent per run_id from run_events ---
  const agentByRunId = new Map<string, string>();
  if (tokensSavedByRunId.size > 0) {
    const runIds = [...tokensSavedByRunId.keys()];
    const agentResult = await ch.query({
      query: `
        SELECT run_id, any(agent) AS agent
        FROM run_events
        WHERE workspace_id = {workspaceId: String}
          AND run_id IN ({runIds: Array(String)})
        GROUP BY run_id
    ...
    const agentRows = await agentResult.json<Record<string, unknown>>();
    for (const r of agentRows) {
      agentByRunId.set(String(r.run_id ?? ""), String(r.agent ?? ""));
    }
  }

  // --- Merge tokens_saved into agent buckets; unresolvable → null overflow ---
  const tokensSavedByAgent = new Map<SavingsAgentName | null, number>();
  for (const [runId, tokensSaved] of tokensSavedByRunId) {
    const rawAgent = agentByRunId.get(runId) ?? "";
    const bucket: SavingsAgentName | null = KNOWN_AGENTS.includes(rawAgent as SavingsAgentName)
      ? (rawAgent as SavingsAgentName)
      : null;
    tokensSavedByAgent.set(bucket, (tokensSavedByAgent.get(bucket) ?? 0) + tokensSaved);
  }

  // Always return explicit rows for the three known agents (AC2)
  const breakdown: AgentSavingsBreakdownRow[] = KNOWN_AGENTS.map((agent) => {
    const cost = costByAgent.get(agent) ?? { totalCostUsd: 0, eventCount: 0 };
    const tokensSaved = tokensSavedByAgent.get(agent) ?? 0;
    return {
      agent,
  ...
  // Overflow row for tokens whose agent could not be resolved
  const overflowTokens = tokensSavedByAgent.get(null) ?? 0;
  if (overflowTokens > 0) {
    breakdown.push({
      agent: null,
      totalCostUsd: 0,
      dollarsSaved: (overflowTokens / 1_000_000) * SAVINGS_RATE_PER_MTOK,
      eventCount: 0,
    });
  }
