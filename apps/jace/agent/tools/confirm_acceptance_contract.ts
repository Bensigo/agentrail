...
export default defineTool({
  description:
    "Confirm a draft Acceptance Contract only after the human explicitly confirms it in this new hosted-channel message. " +
    ...
    "Confirmation enables later Context Pack compilation and builder handoff but never starts implementation, edits code, creates a PR, or merges.",
  ...
  async execute(input, ctx) {
    return confirmAcceptanceContractFromBoundIntake({ sessionAuth: ctx?.session?.auth, version: input.version, env: process.env, transport: realTransport });
  },
});
