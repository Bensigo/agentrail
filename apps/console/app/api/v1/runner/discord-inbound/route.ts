 ...
 * and, on an admitted MESSAGE_CREATE (mentions the bot, or a DM — never the
 * bot's own messages, other bots, or empty content; see
 * apps/jace/agent/lib/discord_gateway.core.mjs's `admitMessage`), POSTs here.
 * Jace has no direct Postgres access (apps/jace is deliberately excluded
 * from this repo's pnpm workspace), so this route is how a Gateway-observed
 * message becomes a `channel_inbox` row — the SAME pipeline the interactions
 * webhook (`app/api/v1/connectors/discord/webhook/route.ts`) already uses
 * for the `/jace` slash command, via the shared `admitDiscordChannelMessage`
 * (`lib/discord-inbound.ts`). One pipeline, two doors — see that route's own
 * header comment for why two doors are unavoidable (different auth models:
 * Discord signs interactions with its own Ed25519 key; this door is
 * server-to-server, authenticated with the shared `JACE_CONSOLE_TOKEN`
 * bearer via `requireJaceConsoleSecret` instead, the same central-secret
 * seam every other Jace-coordinator route uses).
 *
