...
5. Bind the PR produced or attached by the selected builder at a canonical
...
9. Keep the final outcome a human decision. Jace has no implementation or
   merge authority; the selected external builder may produce or attach the
   PR, while Jace binds, verifies, and records the decision seam.
...
Implementation and proof status are intentionally not duplicated in this ADR.
[The trust-layer migration ledger](../trust-layer-migration-ledger.md) is the
canonical current authority; newer ledger and current-main behavior supersede
historical implementation inventories here. Read and report its proof levels
separately: source/test, local runtime, deployed/live, and customer. Source/test
or local-runtime evidence does not establish deployed/live or customer proof.
