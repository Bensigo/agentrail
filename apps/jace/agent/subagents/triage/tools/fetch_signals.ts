// fetch_signals — one of the debugger's FOUR deep-mode evidence-verb tools
...
// Wave 2: .superpowers/sdd/plan-providers.md): "what does the system's
// numeric health look like in this window" — error rates, latency,
// saturation, throughput — across every credentialed `signals`-shaped
// provider for this workspace. Thin zod-input wrapper over the shared
// `lib/evidence_verbs.core.mjs`; `fetch_changes.ts`/`search_events.ts`/
// `fetch_traces.ts` (this file's siblings) differ solely in the literal
...
// does NOT re-format series data — the rendered excerpt IS the adapter's own
// pre-aggregated signal lines (plan-providers.md's pinned convention: one
// line per series/metric, `signal {name}{labels} window_agg=... value=...
// at=...`), which is a provider-adapter concern, not this tool's.
//
// WHEN TO CALL THIS YOURSELF vs. dispatching the nested `anomaly`
// investigator (Task 10): a single narrow discriminating question ("did p99
// latency spike after the deploy?") is cheaper and faster as a direct call
// here. A full baseline-deviation sweep — RED/USE triage, signature
// extraction, first-deviation ordering — is the `anomaly` investigator's
// job, not this tool's; see instructions.md's Deep mode section.
//
...
export default defineTool({
  description:
    ...
    "throughput — across every credentialed signals provider for this workspace. Call this DIRECTLY for a " +
    "single narrow discriminating question during a round; for a full baseline-deviation sweep, dispatch the " +
    "nested anomaly investigator instead. windowStart/windowEnd (ISO-8601, both REQUIRED — every evidence " +
    "query is scoped to a window) bound the search; scope optionally narrows within a provider (e.g. a " +
    "service name); query optionally narrows which metric/signal comes back; limit caps how much comes back " +
    "before the envelope's own size caps. " +
    "\n\n" +
