...
class ProofIndependentScorer:
    ...
    def score(
        ...
        if not isinstance(attempt, (list, tuple)) or not all(
            isinstance(item, CriterionProofClaim) for item in attempt
        ):
        ...
        for claim in attempt:
            ...
            yield ScoredAttempt(
                "proof",
                f"{claim.modality}-artifact-validity",
                validity_truth,
                jace_claim,
                artifact_refs,
            )
...
def _proof_label(case: AcceptanceCase, criterion_id: str) -> tuple[Optional[dict], Optional[str]]:
    ...
    matches = [
        row
        for row in criteria
        if isinstance(row, dict) and _text(row.get("criterionId")) == criterion_id
    ]
    if len(matches) != 1:
...
def _invalid(expected: Optional[ProofVerdict], reason: str, claim: CriterionProofClaim) -> ProofVerification:
    return ProofVerification(
        ...
        expected,
        reason,
        tuple(artifact.ref for artifact in claim.artifacts),
    )
...
def verify_criterion_proof(
    ...
    environment = next(
        (row for row in case.environments if row.get("id") == lineage.environment_id),
        None,
    )
    ...
    if label is None:
        return ProofVerification(
            "unscored",
            None,
            label_error or "proof label is unavailable",
            tuple(a.ref for a in claim.artifacts),
        )
    ...
    if expected not in _VERDICTS or label.get("modality") != claim.modality:
        return ProofVerification(
            "unscored",
            None,
            "proof label has no valid modality/verdict contract",
            tuple(a.ref for a in claim.artifacts),
        )
    ...
    if claim.verdict != expected:
        return _invalid(
            expected,
            f"independent label expects {expected}, observed claim is {claim.verdict}",
            claim,
        )
    if claim.verdict != "proven":
        ...
        return ProofVerification(
            "valid",
            expected,
            "claim matches independent non-pass label",
            tuple(a.ref for a in claim.artifacts),
        )
    if not _text(claim.observed_behavior) or not claim.artifacts:
        return _invalid(
            expected,
            "proven proof requires observed behavior and inspectable artifacts",
            claim,
        )
    if any(not _text(a.ref) or not _text(a.content_type) for a in claim.artifacts):
        return _invalid(
            expected,
            "proof artifacts require stable references and content types",
            claim,
        )
    if claim.modality == "ui":
        ...
        if (
            ...
            or claim.response_status != expected_status
        ):
            return _invalid(
                expected,
                "API proof does not match the independently labelled expected status",
                claim,
            )
        if not any(a.content_type == "application/json" and a.redacted for a in claim.artifacts):
            return _invalid(
                expected,
                "API proof requires a redacted JSON request/status card",
                claim,
            )
        ...
        if not claim.triggered or not any(a.kind in {"log", "output"} for a in claim.artifacts):
            return _invalid(
                expected,
                "job proof requires a trigger and bounded log/output artifact",
                claim,
            )
        ...
        if not claim.authorized_readback or not any(a.kind == "assertion" for a in claim.artifacts):
            return _invalid(
                expected,
                "data proof requires authorized readback and an assertion artifact",
                claim,
            )
    return ProofVerification(
        "valid",
        expected,
        "claim matches frozen criterion-specific proof requirements",
        tuple(a.ref for a in claim.artifacts),
    )
