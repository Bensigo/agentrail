...
- New console routes (auth as the runner surface): `POST /api/v1/runner/preview-boots` (Jace requests: `{eveSessionId, repo, prNumber, headSha}` → creates row keyed like review jobs, enqueues), `GET /api/v1/runner/preview-boots/:id` (Jace polls: `pending|booting|ready{url,bootLogKey?}|failed{reason,bootLogKey?}|torn_down{bootLogKey?}`), fleet-side claim/report mirrors the work-item loop. Worker reports may include a bounded `bootLog` tail; the console uploads it best-effort as `text/plain` under `review-evidence/<workspace>/<repo>/<pr>/<head>/boot.log` and returns the persisted key on GET. TTL: a ready boot lives 20 minutes or until the review job completes, whichever first; teardown is the fleet killing the process group + deleting the directory, always.
