...
def acceptance_lineage(
    ...
    *,
    ...
    _require_arm(arm)
    head = str(pr_head or "").strip()
    environment = str(environment_id or "").strip()
    if not head:
        raise ValueError("Acceptance Case lineage requires an exact PR head")
    if not environment:
        raise ValueError("Acceptance Case lineage requires an environment id")
    revision = next((row for row in case.pr_revisions if row["headSha"] == head), None)
    if revision is None:
        raise ValueError("PR head is not a frozen Acceptance Case revision")
    if not any(row["id"] == environment for row in case.environments):
    ...
    return AcceptanceLineage(
        ...
        pr_head=head,
        diff_identity=str(revision["diffIdentity"]),
