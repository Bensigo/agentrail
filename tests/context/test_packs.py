"""Tests for budget-trimming logic in build_context_pack (issue #706).

AC6:
  (a) over-budget pack is trimmed to ≤ budget with correct itemsDropped
  (b) under-budget pack is untouched (itemsDropped=0)
  (c) unknown-model warning path
  (d) required-context items are never dropped
"""
