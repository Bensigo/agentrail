...
// (Task 6): claimReviewJob / completeReviewJob, hitting the routes Task 4
// built (apps/console/app/api/v1/runner/review-jobs/{claim,complete}/route.ts).
...
// ---------------------------------------------------------------------------
// resolveConsoleConfig / buildClaimUrl / buildCompleteUrl
// ---------------------------------------------------------------------------
...
test("buildClaimUrl / buildCompleteUrl join the trimmed base with the fixed runner paths", () => {
...
test("claimReviewJob: POSTs {workerId, eveSessionId} with a bearer header to the claim URL", async () => {
  const transport = fakeTransport(() => jsonResponse(204, null));
  await claimReviewJob({ workerId: "review-worker-host-1", eveSessionId: "sess-1", env: ENV, transport });
  ...
  assert.deepEqual(JSON.parse(init.body), { workerId: "review-worker-host-1", eveSessionId: "sess-1" });
});

test("claimReviewJob: 204 -> null (nothing eligible)", async () => {
  const transport = fakeTransport(() => jsonResponse(204, null));
  const result = await claimReviewJob({ workerId: "w1", eveSessionId: "s1", env: ENV, transport });
...
test("claimReviewJob: non-2xx (401) throws, mentioning the status", async () => {
  ...
  await assert.rejects(
    () => claimReviewJob({ workerId: "w1", eveSessionId: "s1", env: ENV, transport }),
    (err) => {
...
test("claimReviewJob: non-2xx (500) throws", async () => {
  ...
  await assert.rejects(() => claimReviewJob({ workerId: "w1", eveSessionId: "s1", env: ENV, transport }));
});
...
test("claimReviewJob: missing console config throws WITHOUT ever calling transport", async () => {
  const transport = fakeTransport(() => jsonResponse(204, null));
  await assert.rejects(
    () => claimReviewJob({ workerId: "w1", eveSessionId: "s1", env: {}, transport }),
