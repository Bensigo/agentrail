...
def test_forged_corpus_object_is_reloaded_from_its_root_before_promotion(
    tmp_path: Path,
) -> None:
    _write_corpus(tmp_path, label_class="synthetic")
    forged = replace(load_acceptance_case_corpus(tmp_path), label_class="independent")
...
def test_manifest_bound_entrypoint_runs_all_cases_and_carries_provenance(
    tmp_path: Path,
) -> None:
    ...
    assert report.corpus_provenance["caseSplits"] == {
        "dev-save": "dev",
        "held-save": "held-out",
    }
